...
def resolve_reference_size_cm(image_folder=None):
    """The real-world size of the ArUco cube used for THIS capture, in cm.

    Typing REFERENCE_REAL_SIZE_CM=14 by hand, as the comment above this
    function used to be the only advice for, has no way to fail loudly: get
    it wrong and every volume is off by exactly (14/10)**3 = 2.744x with
    nothing in the run's output to say so (see docs/progress.md, and the
    2026-08-30 run where this happened for real -- inputs/small_leg came
    back at 392.56 cm3 instead of ~1074-1082, silently). A person has to
    remember it every single time; a file sitting next to the images the
    capture actually is only has to be written once.

    Priority, highest first:
      1. REFERENCE_REAL_SIZE_CM env var -- manual override, e.g. for testing
         a value that hasn't been written to a file yet.
      2. `<image_folder>/reference_size_cm.txt` -- the capture's own record
         of which physical cube was in frame. Shipped for the three capture
         folders that need it: inputs/small_leg, inputs/short_leg,
         inputs/est_325 (all three used the 14 cm handmade cube; every
         capture from Aug 2026 on used the 10 cm printed one and needs no
         file at all).
      3. REFERENCE_REAL_SIZE_CM, the 10.0 cm default for current hardware.
    """
    env = os.environ.get("REFERENCE_REAL_SIZE_CM")
    if env is not None:
        return float(env)
    if image_folder:
        override_path = os.path.join(image_folder, "reference_size_cm.txt")
        if os.path.isfile(override_path):
            ...
            try:
                ...
                print(f"    ** WARNING: {override_path} contains {text!r}, "
                      f"not a number -- ignoring it, using the "
                      f"{REFERENCE_REAL_SIZE_CM} cm default instead")
    return REFERENCE_REAL_SIZE_CM
