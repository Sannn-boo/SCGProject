...
def detect_top_k_objects(pcd, k=2, visualize=False):
    ...
    if has_marker:
        # The cluster carrying the ArUco checkerboard pattern (high aruco_score)
        # IS the reference box -- the box is physically printed with ArUco
        # markers on every face (see config.py REFERENCE_MARKER_DICT). This used
        # to read `box_idx = 1 - marker_idx`, which assigned "box" to the OTHER
        # cluster -- i.e. the limb -- every time this branch fired. The fallback
        # branch below never had this bug: it already assigns the highest-
        # cubeness cluster to box_idx *directly*, with no inversion. Fixed
        # 2026-08-29.
        marker_idx = int(np.argmax(aruco_scores))
        box_idx = marker_idx
        print(f"    ArUco marker detected at cluster #{marker_idx+1} → that is the box")
    else:
    ...
    # Sanity check, not a hard gate: the box should look more like a cube than
    # the obj does. This class of bug (box/obj swapped) has no other symptom --
    # the pipeline runs to completion and reports a plausible-looking wrong
    # number -- so a loud warning here is cheap insurance against it recurring
    # silently, on this branch or a future one.
    box_cubeness = info[box_idx][4]
    obj_cubeness = info[obj_idx][4]
    if box_cubeness < obj_cubeness:
        print(f"    ** WARNING: box cluster (cubeness={box_cubeness:.3f}) is LESS "
              f"cube-shaped than the obj cluster (cubeness={obj_cubeness:.3f}). "
              f"box/obj may be swapped -- check the downstream volumes before "
              f"trusting this run.")
