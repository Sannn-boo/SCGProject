...
def _resolve_device(device):
    """cuda ▸ mps ▸ cpu, via the same picker the rest of the pipeline uses.

    Every function below used to default to device="cuda" outright. On a
    machine with a real NVIDIA GPU that default is invisible -- it's always
    right -- which is exactly how it shipped without anyone noticing it was
    hardcoded. On Apple Silicon (or any CPU-only box) every call silently
    tried to allocate CUDA and failed. One caller (prep.py's cube-box
    widening) happens to catch that and fall back; _leg_mask() does not, so
    the same root cause showed up as a clean fallback in one place and a
    crash in the other. Passing device=None (the new default) resolves to
    whatever vggt/utils/device.get_device() finds -- the identical check
    that already picks "mps" correctly for the main VGGT model on this
    machine. An explicit device="cuda"/"mps"/"cpu" from a caller still wins.
    """
...
def _to_device_safe(inputs, device, torch):
    """Move a processor's output to `device`, casting float64 -> float32 first.

    MPS refuses float64 outright: "Cannot convert a MPS Tensor to float64
    dtype, the MPS framework doesn't support float64." CUDA and CPU accept
    it silently, which is why this never surfaced before this file got MPS
    support -- SAM's processor builds some of its output tensors (observed:
    the ones derived from `input_boxes`) as float64 because that is numpy's
    default float dtype, not because anything asked for double precision.

    Only float64 tensors are touched; integer bookkeeping like
    original_sizes/reshaped_input_sizes -- read later via inputs[...] --
    passes through with its dtype exactly as the processor built it. SAM and
    DINO's own weights are float32 already (see _models()), so this is a
    dtype-compatibility fix, not a precision change.
    """
...
def detect(image_pil, prompt, threshold=DETECT_THRESHOLD, device=None):
    """Best box for `prompt`, as [x0, y0, x1, y1], or None."""
    device = _resolve_device(device)
    m = _models(device)
    torch = m["torch"]
    inputs = _to_device_safe(m["dino_proc"](images=image_pil, text=prompt,
                                            return_tensors="pt"), device, torch)
    with torch.no_grad():
        out = m["dino"](**inputs)
...
def segment(image_pil, box, device=None):
    """Mask of whatever `box` encloses, as a boolean array."""
    device = _resolve_device(device)
    m = _models(device)
    torch = m["torch"]
    inputs = _to_device_safe(
        m["sam_proc"](image_pil, input_boxes=[[list(map(float, box))]],
                      return_tensors="pt"), device, torch)
    with torch.no_grad():
