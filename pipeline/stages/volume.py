...
def compute_volumes(object_mesh_paths: list[str],
    ...
    ref_size_cm: real-world size of the reference cube, in cm. Defaults to
    REFERENCE_REAL_SIZE_CM (module import time), but callers should pass
    resolve_reference_size_cm(image_folder) instead so the right cube size
    is picked up per capture automatically -- see that function's docstring
    in pipeline/config.py.
    """
    if ref_size_cm is None:
        ref_size_cm = REFERENCE_REAL_SIZE_CM
    res_label = "auto" if auto_res else str(voxel_res)
    print()
    print("=" * 60)
    print(f"STAGE 7: real-world volumes  "
          f"(ref = {ref_size_cm} cm ArUco cube  |  voxel_res={res_label})")
    print("=" * 60)
...
# ---------------------------------------------------------------------------
# An alternative calibration -- fitted reference faces + a marker
# cross-check, instead of the volume ratio above -- is archived at
# pipeline/_parked/stage6_m10_candidate.py, not imported here. It scores
# worse against water-displacement ground truth (4.1% vs 1.7% mean absolute
# error) so it is not the default, but the reasoning and the marker
# cross-check are still worth reading before touching this stage's
# calibration. See pipeline/_parked/README.md.
# ---------------------------------------------------------------------------
