...
def main():
    ...
    try:
        ...
        if getattr(args, "prep", True):
            # This used to call prepare_frames(...) directly, with its own
            # hand-copied argument list -- the exact spot where this entry
            # point and stagerun.py's run_stage0() diverged before (see
            # pipeline/stage0_prep.py's docstring). Both now call the same
            # function, so that class of bug can't recur here.
            from pipeline.stage0_prep import run_prep_stage
            prep_images, manifest = run_prep_stage(
                args.image_folder, stage_dirs[0], args)
        ...
        if vol_objects:
            ...
            from pipeline.config import resolve_reference_size_cm
            vol_df = compute_volumes(vol_objects,
                                     voxel_res=args.voxel_res,
                                     auto_res=args.auto_res,
                                     ref_size_cm=resolve_reference_size_cm(
                                         args.image_folder))
